...
size_t idFileSystemLocal::CurlWriteFunction( void *ptr, size_t size, size_t nmemb, void *stream ) {
	...
	#ifdef _WIN32
		return _write(1, ptr, size * nmemb);
		//return _write( static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr()->_Placeholder, ptr, size * nmemb );
	#else
...
dword BackgroundDownloadThread( void *parms ) {
	while( 1 ) {
		...
		if ( bgl->opcode == DLTYPE_FILE ) {
			...
			#if defined(WIN32)
				_read(1, bgl->file.buffer, bgl->file.length);
				//_read( static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr()->_Placeholder, bgl->file.buffer, bgl->file.length );
			#else
