...
void CTextureBar::OnDeltaposSpinHshift(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  if (pNMUpDown->iDelta < 0)
    Select_ShiftTexture(abs((long)g_qeglobals.d_savedinfo.m_nTextureTweak), 0);
  else
    Select_ShiftTexture(-abs((long)g_qeglobals.d_savedinfo.m_nTextureTweak), 0);
...
void CTextureBar::OnDeltaposSpinVshift(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
  if (pNMUpDown->iDelta < 0)
    Select_ShiftTexture(0, abs((long)g_qeglobals.d_savedinfo.m_nTextureTweak));
  else
    Select_ShiftTexture(0, -abs((long)g_qeglobals.d_savedinfo.m_nTextureTweak));
  GetSurfaceAttributes();
}
...
void CTextureBar::OnDeltaposSpinVScale(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
  if (pNMUpDown->iDelta < 0)
	  Select_ScaleTexture(0, (float)abs((long)g_qeglobals.d_savedinfo.m_nTextureTweak));
  else
	  Select_ScaleTexture(0, (float)-abs((long)g_qeglobals.d_savedinfo.m_nTextureTweak));
  GetSurfaceAttributes();
}
